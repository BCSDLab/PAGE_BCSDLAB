  ...
  <div class="container">
    ...
    <!-- <div class="information-section">
      ...
      <div class="information__list">
        <div
          class="information"
          v-for="(info, index) in informationDatas"
          :key="index">
          <div class="information__title">{{ info.title }}</div>
          <hr>
          <div
            v-html="info.contents"
            class="information__contents">
          </div>
        </div>
      </div>
    </div> -->
    <div class="position-section">
      <div class="title">Recruit position</div>
      <position-card />
    </div>
    <div class="recruit-process-section">
      <div class="title">Recruit Process</div>
      ...
      <div class="processes">
        <div
          ...
          <div
            v-html="process.contents"
            class="process__contents">
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="join">
      <div class="title">
        BCSD Lab과 함께 하실 준비가 되었나요?
      </div>
      <div class="description">
        BCSD Lab과 함께 할 코리아텍 학우 여러분들을 환영합니다.<br>
        함께 프로젝트를 이어나가고, 자신의 능력을 향상시키고 싶은 분들의 많은 지원바랍니다.
      </div>
      <button
        @click="apply()"
        class="apply-btn">
  ...
  export default {
    ...
    data () {
      return {
        ...
        processDatas: [
          { step: '1', title: '지원서 접수', contents: '본인의 역량과 관심사에 따라<br>Novice와 Beginner를 선택합니다' },
          { step: '2', title: '지원서 검토', contents: '개발자는 코딩테스트를 수행하고<br>디자이너는 포트폴리오를 살펴보며<br>지원자의 수준을 파악하고 어디부터<br>배워야할지 검토합니다<br>(단, Novice는 코딩테스트 제외)' },
          { step: '3', title: '커리큘럼 진행', contents: 'Novice는 공통 과정을<br>Beginner는 트랙별 교육을 진행합니다<br>해당 교육과정은 각 직군 별로 상이하며약 6개월 간 진행됩니다' },
    ...
    methods: {
      apply() {
        window.open("http://bitly.kr/GWZKcwD")
      },
      inquiry () {
...
<style scoped>
  ...
  .information__list .information:last-child {
    margin-right: 0;
  }
  ...
  .process-bar {
    height: 18px;
    width: 100%;
    background: linear-gradient(to right,#c5d4e3, #54606e);
    display: flex;
    justify-content: center;
    align-items: center;
    margin-top: 128px;
  ...
  .process-bar .circle:first-child {
    margin-left: -154px;
  }

  @media (max-width: 1324px) {
    .process-bar .circle:last-child {
      margin-right: 0;
    }
    .process-bar .circle:first-child {
      margin-left: -268px;
  ...
  .advantage__list .advantage:last-child {
    margin-right: 0;
  }
